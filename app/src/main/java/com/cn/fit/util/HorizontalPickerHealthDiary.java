 ...
 */
@SuppressLint("NewApi")
public class HorizontalPickerHealthDiary extends View {
...
//    public static final String TAG = "HorizontalTimePicker";
    ...
    public HorizontalPickerHealthDiary(Context context) {
        this(context, null);
    }

    public HorizontalPickerHealthDiary(Context context, AttributeSet attrs) {
        this(context, attrs, R.attr.horizontalPickerStyle);
    }

    public HorizontalPickerHealthDiary(Context context, AttributeSet attrs, int defStyle) {
        ...
        setValues(values);
        setSideItems(sideItems, screenWidth, screenHeight);
    ...
    protected void onMeasure(int widthMeasureSpec, int heightMeasureSpec) {
        ...
        setMeasuredDimension(width, (int) (1.5 * height));
    }
...
//            BoringLayout layout = mLayouts[i];
            StaticLayout layout = mLayouts[i];
...
//                        mItemWidth);
                SpannableString abc = new SpannableString(mValues[i]);
                if (mValues.length > 3) {
                    if (i != mSelectedItem) {
                        //                    abc.setSpan(new AbsoluteSizeSpan(20, false), 0, 2, Spanned.SPAN_MARK_MARK);
                        //                    abc.setSpan(new AbsoluteSizeSpan(10, false), abc.length()-2, abc.length(), Spanned.SPAN_MARK_MARK);
                        abc.setSpan(new RelativeSizeSpan(0.8f), 0, 2, Spanned.SPAN_MARK_MARK);
                        abc.setSpan(new RelativeSizeSpan(0.4f), abc.length() - 3, abc.length(), Spanned.SPAN_MARK_MARK);
                    } else {
                        abc.setSpan(new StyleSpan(android.graphics.Typeface.BOLD), 0, 2, Spanned.SPAN_MARK_MARK);
                        abc.setSpan(new StyleSpan(android.graphics.Typeface.BOLD), abc.length() - 3, abc.length(), Spanned.SPAN_MARK_MARK);
                        //                    abc.setSpan(new AbsoluteSizeSpan(30, false), 0, 2, Spanned.SPAN_MARK_MARK);
                        //                    abc.setSpan(new AbsoluteSizeSpan(15, false), abc.length()-2, abc.length(), Spanned.SPAN_MARK_MARK);
                        abc.setSpan(new RelativeSizeSpan(1.2f), 0, 2, Spanned.SPAN_MARK_MARK);
                        abc.setSpan(new RelativeSizeSpan(0.6f), abc.length() - 3, abc.length(), Spanned.SPAN_MARK_MARK);
                    }
                }
                mLayouts[i] = new StaticLayout(abc, mTextPaint, mItemWidth, Layout.Alignment.ALIGN_CENTER, 0.8f, 1f, false);
            }
        }
    ...
    public void setSideItems(int sideItems, int width, int height) {
        if (mSideItems < 0) {
            ...
            calculateItemSize(screenWidth, screenWidth);
        }
    }
...
//            mLayouts = new BoringLayout[mValues.length];
            mLayouts = new StaticLayout[mValues.length];
            for (int i = 0; i < mLayouts.length; i++) {
...
//            e.printStackTrace();
    ...
    private static final class Marquee extends Handler {
        ...
        private final WeakReference<HorizontalPickerHealthDiary> mView;
        ...
        Marquee(HorizontalPickerHealthDiary v, Layout l, boolean rtl) {
            ...
            mView = new WeakReference<HorizontalPickerHealthDiary>(v);
        ...
        void tick() {
            ...
            final HorizontalPickerHealthDiary view = mView.get();
...
//            dest.writeArray(mvalue);
    ...
    private static class PickerTouchHelper extends ExploreByTouchHelper {
        ...
        private HorizontalPickerHealthDiary mPicker;

        public PickerTouchHelper(HorizontalPickerHealthDiary picker) {
            super(picker);
            mPicker = picker;
        }
