 ...
 */
package com.cn.fit.ui.chat.ui.plugin;
...
public class FileExplorerActivity extends ECSuperActivity implements
    ...
    public void onCreate(Bundle savedInstanceState) {
        ...
        if (TextUtils.isEmpty(title)) {
            getTopBarView().setTopBarToStatus(1, R.drawable.ic_action_navigation_arrow_back_inverted, -1, R.string.plugin_file_explorer_ui_title, this);
        } else {
            getTopBarView().setTopBarToStatus(1, R.drawable.ic_action_navigation_arrow_back_inverted, -1, title, this);
        }
    ...
    private void setCurrentTabSelector(int type) {
        mType = type;
        if (type == DIR_SDCARD) {
            mSdcardTab.setTextColor(getResources().getColor(R.color.blue_second));
        ...
        mRootTab.setTextColor(getResources().getColor(R.color.blue_second));
