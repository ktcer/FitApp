package com.cn.fit.ui.scancode;
...
import com.cn.fit.ui.scancode.camera.CameraManager;
import com.cn.fit.ui.scancode.common.BitmapUtils;
import com.cn.fit.ui.scancode.decode.BitmapDecoder;
import com.cn.fit.ui.scancode.decode.CaptureActivityHandler;
import com.cn.fit.ui.scancode.view.ViewfinderView;
 ...
 * @author Sean Owen
 */
public final class ActivityCapture extends ActivityBasic implements
    ...
    public void onCreate(Bundle savedInstanceState) {
        ...
        findViewById(R.id.common_title).setBackgroundColor(getResources().getColor(R.color.transblack2));
        // 监听图片识别按钮
        findViewById(R.id.capture_scan_photo).setOnClickListener(this);
        ((TextView) findViewById(R.id.middle_tv)).setText("扫一扫");
    ...
    public void handleDecode(Result rawResult, Bitmap barcode, float scaleFactor) {
        ...
        //已经识别出内容之后
        String[] content = ResultParser.parseResult(rawResult).toString().split(",");
//		ArrayList<String> list = new ArrayList<String>();
//		for (int i = 0; i < content.length; i++) {
//			list.add(content[i]);
//		}
        try {
            if (content[0].equals("codeType=1")) {// addexpert表示加专家
//				Gson gson = new Gson();
//				BeanTwoCode beanTwoCode = new BeanTwoCode();
//				beanTwoCode = gson.fromJson("{"+ResultParser.parseResult(rawResult).toString()+"}",
//						new TypeToken<BeanTwoCode>() {
//						}.getType());
                String[] id = content[1].split("=");
                String[] imag = content[3].split("=");
                Intent intent = new Intent(
                        ActivityCapture.this,
                        ActivityCoachPage.class);
                intent.putExtra("ExpertId", Long.parseLong(id[1]));
                intent.putExtra("image", imag[1]);
                intent.putExtra("pageType", "1");
                startActivity(intent);
            } else {
                ToastUtil.showMessage("目前只能使用扫码功能添加健康秘书哦~");
            }
