...
public class ScrollLayout extends LinearLayout {
    ...
    private void drawUnableText() {

        for (int i = getChildCount() - 1; i != -1; i += -1) {
            TimeView tv = (TimeView) getChildAt(i);
            int index = i;
            if (index >= 0 && index < getChildCount()) {
                tv.setVals((TimeView) getChildAt(index));
            } else {
                tv.setVals(mLabeler.add(tv.getEndTime(), 0));
            }
    ...
    public boolean onTouchEvent(MotionEvent ev) {
        ...
        switch (action) {
            ...
            case MotionEvent.ACTION_MOVE:
                isTouch = true;
                mScrollX += mLastX - x;
                if (Math.abs(mScrollX) >= objWidth) {
                    int distance = (mScrollX / objWidth) * objWidth;
                    reScrollTo(distance, 0, true);
                } else {
//                    reScrollTo(mScrollX, 0, true);
                    reScrollTo(mLastX, 0, true);
                }
                break;
            case MotionEvent.ACTION_UP:
                System.out.println("我执行了up事件");
                isTouch = false;
