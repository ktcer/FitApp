...
//        mediaRecorder.setVideoFrameRate(16);
     ...
     * 播放视频
     */
    public void playVideo1(File vecordFile1) {
        surfaceView.setVisibility(View.GONE);
        videoSurfaceView.setVisibility(View.VISIBLE);
        playVideo.setVisibility(View.GONE);
        mediaPlayer = new MediaPlayer();
        try {
            mediaPlayer.reset();
            mediaPlayer.setDataSource(vecordFile1.getAbsolutePath());
            mediaPlayer.setDisplay(videoSurfaceView.getHolder());
            mediaPlayer.prepare();//缓冲
            mediaPlayer.start();
        } catch (Exception e) {
    ...
    private void createRecordDir() {
        ...
        String basicFolder = AppDisk.appInursePath + UtilsSharedData.getValueByKey(Constant.USER_ACCOUNT) + File.separator;
        File sampleDir = new File(basicFolder + AppDisk.DCIM_VIDEO);
        if (!sampleDir.exists()) {
            sampleDir.mkdirs();
        }
//        File vecordDir = sampleDir;
        // 创建文件
//        try {
//        	vecordFile = new File(basicFolder+AppDisk.DCIM_VIDEO+"1.mp4");
//        if(getRecorderListener()!=null){
//        	vecordFile = getRecorderListener().reName();
//        	if(vecordFile==null)
//        }
        vecordFile = new File(basicFolder + AppDisk.DCIM_VIDEO + "视频提醒.mp4");
//        if(getRecorderListener()!=null){
//        	vecordFile = getRecorderListener().reName();
//        	if(vecordFile==null)
//        		vecordFile = new File(basicFolder+AppDisk.DCIM_VIDEO+"视频提醒.mp4");
//        }
//            vecordFile = File.createTempFile("视频提醒", ".mp4", vecordDir);//mp4格式
//        } catch (IOException e) {
//        }
    }
